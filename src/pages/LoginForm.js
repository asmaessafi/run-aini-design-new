...
const LoginForm = () => {
    ...
    // );
            ...
            if (userData) {
                // Check if user data already has profile structure
                const storedUser = JSON.parse(localStorage.getItem('user') || '{}');
                
                // Transform user data to profile structure, preserving existing data
                const profileData = transformUserDataToProfile(
                    { ...storedUser, ...userData }, // Merge existing with new data
                    {}
                );
